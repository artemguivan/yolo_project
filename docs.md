...
```
show_flag: переменная, позволяющая в отдельном окне отображать обработку видео, чтобы отображать
поменяйте значение в show_flag на True, по умолчанию False
